# from ibrnet feature_network.py
# NOTE if you want to use this
# get model weights from https://drive.google.com/file/d/1wNkZkVQGx7rFksnX7uVX3NazrbjqaIgU/view?usp=drive_link
...
class ResUNet(nn.Module):
    ...
    def forward_nhwc(self, x):
        ...
        coarse, fine = self.forward(x)
        return coarse.permute(0,2,3,1).contiguous().detach(), fine.permute(0,2,3,1).contiguous().detach()


def load_feature_net():
    filename = "models/model_255000.pth"
    to_load = torch.load(filename)
    
    coarse_feat_dim = 32        # from ibrnet/config.py
    fine_feat_dim   = 32        # from ibrnet/config.py
    coarse_only     = False     # from ibrnet/config.py
    feature_net = ResUNet(coarse_out_ch=coarse_feat_dim, fine_out_ch=fine_feat_dim, coarse_only=coarse_only)
    feature_net.load_state_dict(to_load['feature_net'])
    
    return feature_net.cuda()